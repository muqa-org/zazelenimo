...
const assertEnv = (val: string | undefined, envName: string) => {
  if (!val || val.includes('your-cometh')) {
    console.warn(
      `Environment variable ${envName} is not properly set. Using fallback value.`,
    );
...
function getConfig(): ComethConfig {
  ...
  const comethProjectChain = (process.env.NEXT_PUBLIC_CHAIN ||
    'ARBITRUM_SEPOLIA') as chainType;
  ...
  if (!chainConfig) {
    console.error(
      `No Cometh configuration found for chain: ${comethProjectChain}. Defaulting to ARBITRUM_SEPOLIA.`,
    );
    return getConfig();
  }
  ...
  const validApiKey = assertEnv(
    apiKey,
    `NEXT_PUBLIC_${comethProjectChain}_COMETH_API_KEY`,
  );
